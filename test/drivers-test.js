...
test('uncompressed ESRI Shapefile', function(t) {
    ...
    ogr2ogr(dir + 'sample.shp').format('ESRI Shapefile').exec(function(er, buf) {
        t.notOk(er, 'no error', { error: er })
...
test('uncompressed ESRI Shapefile without shx', function(t) {
    ...
    ogr2ogr(dir + 'sample.lonely.shp').format('ESRI Shapefile')
        .options(["--config", "SHAPE_RESTORE_SHX", "TRUE"])
        .exec(function(er, buf) {
            t.notOk(er, 'no error', { error: er })
...
test('PostgreSQL', function(t) {
    ogr2ogr(dir + 'sample.geojson')
        ...
        .exec(function(er) {
            t.ok(/Connection refused/.test(er.message) || /password authentication failed/.test(er.message), 'should try to connect to postgres');
            t.end()
        })
})
