...
let sampleCsvNogeom = join(__dirname, '/samples/sample-nogeom.csv')
let sampleJson = join(__dirname, '/samples/sample.json')
let sampleNestedZip = join(__dirname, '/samples/sample.shp.nested.zip')

let bufferStream = function (st, cb) {
  let data = []
...
test('api input formats', function (tp) {
  ...
  tp.test('accepts a GeoJSON object', function (t) {
    ...
    let ogr2 = ogr2ogr(geojson).format('shp')
    let st = ogr2.stream()
  ...
  tp.test('accepts a stream', function (t) {
    ...
    let stream = fs.createReadStream(sampleKml)
    let ogr = ogr2ogr(stream, 'kml')
...
test('api output formats', function (tp) {
  tp.test('returns a stream', function (t) {
    ...
    let st = ogr2ogr(sampleKml).stream()
    bufferStream(st, function (buf) {
...
test('errors when converting', function (t) {
  ...
  let st = ogr2ogr(sampleJson).format('shp').stream()
  st.on('error', function (er) {
    t.ok(er, 'expect error', {error: er})
  })
})
...
test('skipfailures option', function (t) {
  let ogr = ogr2ogr(sampleNestedZip).skipfailures()
